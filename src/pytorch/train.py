...
def train(train_loader, tnet, criterion, optimizer, epoch):
    ...
    for batch_idx, (data1, data2, data3) in enumerate(train_loader):
        # if args.cuda:
        data1, data2, data3 = data1.float().cuda(), data2.float().cuda(), data3.float().cuda()
...
def save_checkpoint(state, is_best, filename='checkpoint_resnet101.pth.tar'):
...
def predict(model_path):
    ...
    for image_name in os.listdir(test_path):
        ...
        if (count%32 == 0 or count == total-1):
            predicts = model.predict(np.array(batch_image))
            test_preds += predicts.to_list()
            test_file_name += batch_file_name.to_list()
            batch_image = []
            batch_file_name = []
        count+=1

    print(np.array(test_preds).shape)
    print(np.array(test_file_name).shape)
...
if __name__ == '__main__':
    predict('./runs/TripletNet/checkpoint.pth.tar')
